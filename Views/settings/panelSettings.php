<div class="panel-section panelSettings" style="z-index: -1;" id="panelSettings" data-status="show">
    <div class="panel-section-title">
        <h1>Paramètres</h1>
    </div>
    <div>
        <div class="panel-section-scroll" id="panelSettings-scroll" data-status="show">
            <div class="panel-section-list">
                <a href="/settings/?tab=account" class="panel-section-button">
                    <div class="icon">
                        <img class="" height="24" src="assets/image/reglages.png" width="24" alt="">
                    </div>
                    <div class="text">
                        <span>Général</span>
                    </div>
                    <div class="hover"></div>
                </a>
                <a href="/settings/?tab=security" class="panel-section-button">
                    <div class="icon">
                        <img class="" height="24" src="assets/image/bouclier.png" width="24" alt="">
                    </div>
                    <div class="text">
                        <span>Sécurité et connexion</span>
                    </div>
                    <div class="hover"></div>
                </a>
            </div>
        </div>
    </div>
</div>